...
-- using the created view num_word_chapter, created a query to get the sum and
-- average of the number of words read each day if one reads 4 chapters everyday
with final_q as 
(
	select 
		t.row,
		t.book, 
		avg(t.num_words) over (
			rows between current row and 3 following
			) as avg_num,
		sum(t.num_words) over (
			rows between current row and 3 following
			) as sum_num
	from (
